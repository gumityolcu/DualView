...
def calc_influence_function(train_dataset_size, grad_z_vecs=None, e_s_test=None):
    ...
    for i in range(train_dataset_size):
        ...
        influences.append(tmp_influence.to('cpu'))
...
def batch_interface(
    ...
    xpl=torch.empty((0,len(train_loader.dataset)))
    for i,x in enumerate(batch):
        tmp_xpl=interface(model,train_loader,x[None,...],targets[None,i],gpu,recursion_depth,r)
        xpl=torch.cat((xpl,tmp_xpl[None,...]),dim=0)
    return xpl
...
def calc_img_wise(config, model, train_loader, test_loader, loss_func="cross_entropy"):
    ...
    for j in range(test_dataset_iter_len):
        ...
        end_time = time.time()
        print(f"{end_time - start_time} seconds with recursion_depth={config['recursion_depth']} and r={config['r_averaging']}")
...
def precompute_interface(model, save_dir, train_loader, test_loader, device):
    """Calculates the influence function by first calculating
    all grad_z, all s_test and then loading them to calc the influence"""

    gpu=0 if device=="cuda" else -1
    s_test_outdir = os.path.join(save_dir,"s_test/")
    if not os.path.exists(s_test_outdir):
        os.makedirs(s_test_outdir)
    grad_z_outdir = os.path.join(save_dir,"grad_z/")
    if not os.path.exists(grad_z_outdir):
        os.makedirs(grad_z_outdir)
    ...
    calc_grad_z(
        model, train_loader, grad_z_outdir, gpu
    )
    ...
    influence_results["harmful"] = harmful
    influence_results["helpful"] = helpful
    influences_path = save_dir.joinpath("influence_results.json")
    save_json(influence_results, influences_path)
