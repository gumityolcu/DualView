...
def s_test(x_test, y_test, model, i, samples_loader, gpu=-1, damp=0.01, scale=25.0, loss_func="cross_entropy"):
    ...
    for i, (x_train, y_train) in enumerate(samples_loader):
        if gpu >= 0:
            x_train, y_train = x_train.cuda(), y_train.cuda()
...
def grad_z(x, y, model, gpu=-1, loss_func="cross_entropy"):
    ...
    return grad(loss, model.if_parameters())
...
def s_test_sample(
    ...
    inverse_hvp = [
        torch.zeros_like(params, dtype=torch.float) for params in model.if_parameters()
    ]
