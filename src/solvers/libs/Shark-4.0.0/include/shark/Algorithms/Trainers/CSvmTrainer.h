...
public:
    ...
    std::pair<RealMatrix,RealMatrix> train_custom(LinearClassifier<InputType>& model, LabeledData<InputType, unsigned int> const& dataset)
    {
        std::size_t classes = numberOfClasses(dataset);
        std::pair<RealMatrix,RealMatrix> ret;
        //RealMatrix ret;
        //if(classes == 2){
            //trainBinary(model,dataset);
            //return;
        ...
        switch (m_McSvmType){
            case McSvm::WW:
                ret= trainMc_custom<QpMcLinearWW<InputType> >(model,dataset,classes);
            case McSvm::CS:
                ret= trainMc_custom<QpMcLinearCS<InputType> >(model,dataset,classes);
                break;
            case McSvm::LLW:
                ret= trainMc_custom<QpMcLinearLLW<InputType> >(model,dataset,classes);
            case McSvm::ATM:
                ret= trainMc_custom<QpMcLinearATM<InputType>>(model,dataset,classes);
            case McSvm::ATS:
                ret= trainMc_custom<QpMcLinearATS<InputType> >(model,dataset,classes);
            case McSvm::ADM:
                ret= trainMc_custom<QpMcLinearADM<InputType> >(model,dataset,classes);
            case McSvm::MMR:
                ret= trainMc_custom<QpMcLinearMMR<InputType> >(model,dataset,classes);
            case McSvm::ReinforcedSvm:
                ret= trainMc_custom<QpMcLinearReinforced<InputType> >(model,dataset,classes);
            //case McSvm::OVA://OVA is a special case and implemented here
             //   trainOVA(model,dataset,classes);
              //  break;
        }
...
private:
    ...
    RealMatrix trainBinary_custom(LinearClassifier<InputType>& model, LabeledData<InputType, unsigned int> const& dataset)
    {
        std::size_t dim = inputDimension(dataset);
        QpBoxLinear<InputType> solver(dataset, dim);
        solver.solve(
        ...
        if(!this->trainOffset()){
            ...
            row(w,0) = solver.solutionWeightVector();
            model.decisionFunction().setStructure(w);
            return solver.solutionLagrangeVector();
        }
    ...
    template<class Solver>
    std::pair<RealMatrix,RealMatrix> trainMc_custom(LinearClassifier<InputType>& model, LabeledData<InputType, unsigned int> const& dataset, std::size_t classes){
        std::size_t dim = inputDimension(dataset);

        Solver solver(dataset, dim, classes);
        return solver.solve_custom(random::globalRng, this->C(), this->stoppingCondition(), &this->solutionProperties(), true);
    }
