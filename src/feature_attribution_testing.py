...
def evaluate(model_name, model_path, device, class_groups,
    ...
    for p in range(pages):
        ...
        # classic: take highest explanations
        #subset_xpl = cumul_xpl[start_ind:start_ind+T]
        # new: take more interesting(?) subset of explanations
        step = 1
        print(cumul_xpl.shape)
        subset_xpl = cumul_xpl[list(range(start_ind, start_ind + step*T, step))]
        if dataset_name=="MNIST":
            out = generate_all_pixel_attributions_MNIST_show(model, train, test, subset_xpl, indices, save_dir, fname, T, device, composite='EpsilonPlus', canonizer=canonizer, modes=["heatmap"], start_ind=start_ind-offset, include_test=True)
        else:
            generate_all_pixel_attributions(model, train, test, subset_xpl, indices, save_dir, fname, T, device, composite='EpsilonPlus', canonizer=canonizer, modes=["heatmap"], start_ind=start_ind-offset, include_test=True)
    return out

def evaluate_with_given_train_test(model_name, model_path, device, class_groups,
             dataset_name, dataset_type,
             data_root, xpl_root,
             save_dir, validation_size, num_classes,imagenet_class_ids, testsplit, pages, train, test):
    if not torch.cuda.is_available():
        device="cpu"
    ds_kwargs = {
        'data_root': data_root,
    ...
    for p in range(pages):
        ...
        step = 1
        print(cumul_xpl.shape)
        subset_xpl = cumul_xpl[list(range(start_ind, start_ind + step*N, step))]
        if dataset_name=="MNIST":
            out = generate_all_pixel_attributions_MNIST_show(model, train, test, subset_xpl, indices, save_dir, fname, T, device, composite='EpsilonPlus', canonizer=canonizer, modes=["heatmap"], start_ind=start_ind-offset, include_test=True)
        else:
            generate_all_pixel_attributions(model, train, test, subset_xpl, indices, save_dir, fname, T, device, composite='EpsilonPlus', canonizer=canonizer, modes=["heatmap"], start_ind=start_ind-offset, include_test=True)
...
def generate_all_pixel_attributions(model, train, test, cumul_xpl, indices, save_dir, fname, T, device, composite, canonizer, modes=["heatmap"],include_test=False,start_ind=0):
    ...
    for mode in modes:
        ...
        if "std" in fname:
            ...
            dstype= "Superlabels"
        fig.savefig(os.path.join(save_dir, f"test_{fname}_{composite}_all_{mode}.pdf"))
        ...
        # FOR WINDOWS
        #os.system(f"pdfcrop {os.path.join(save_dir, f'{fname}_{composite}_all_{mode}.pdf')}")
        os.system(f"del {os.path.join(save_dir, f'test_{fname}_{composite}_all_{mode}.pdf')}")
        os.system(f"move {os.path.join(save_dir, f'test_{fname}_{composite}_all_{mode}-crop.pdf')} {os.path.join(save_dir, f'{fname}_{composite}_all_{mode}.pdf')}")
...
def generate_all_pixel_attributions_MNIST_show(model, train, test, cumul_xpl, indices, save_dir, fname, T, device, composite, canonizer, modes=["heatmap"],include_test=False,start_ind=0):
    #alpha=0.2
    #vis_th=0.35
    alpha=0.
    vis_th=0.    
    ...
    for ind in indices:
        x, y = test[ind]
        test_img = test.inverse_transform(x)
        test_label = y
        if isinstance(y,tuple):
            y=y[1]
        y = torch.tensor([y], dtype=torch.int)
    ...
    for mode in modes:
        ...
        fig.show()
        #print(xpl_list)
        return test_img, test_label, xpl_list
